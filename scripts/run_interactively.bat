...
ssh -tt -J %user%@labta.math.unipd.it %user%@labsrv8.math.unipd.it "srun --job-name=SD1.4 --chdir=/home/dvezzaro/hf_sd --partition=testing --time=0-04:00:00 --mem=12G --constraint=A6000 --gres=gpu --cpus-per-task=4 --pty bash -c 'source /conf/shared-software/anaconda/etc/profile.d/conda.sh && conda activate hf_sd && python -W ignore /home/dvezzaro/hf_sd/run_sd.py && conda deactivate'"